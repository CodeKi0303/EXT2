...
#define NUMBER_OF_SECTORS		16385
...
static COMMAND g_commands[] =
{
    ...
    { "memdump",shell_cmd_memdump,    COND_MOUNT    },
...
int shell_cmd_memdump( int argc, char* argv[] )
{
    printf("Start memony dump\n");
    printf("start address %x : ",&g_disk);
    printf("\n");
    int i,j,k;
    char* gd_offset=(char*)(&g_disk);
    gd_offset=gd_offset+512;
    for(i=0;i<10;i++)
    {
        for(j=0;j<16;j++)
        {
            unsigned char bte=gd_offset[j];
            printf("%02x ",bte);
        }
        gd_offset=gd_offset+16;
        printf("\n");
    }
    /*
    for(i=0;i<10;i++)
